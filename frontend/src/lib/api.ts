...
export async function fetchRecipe(recipeId: string): Promise<RecipeCardData> {
  ...
  return {
    ...
    tags: normalizeIngredients(payload.tags),
    calories: null,
    ingredients: normalizeIngredients(payload.ingredients),
    instructions: normalizeIngredients(payload.instructions),
...
function normalizeRecipeRecord(input: Record<string, unknown>): RecipeCardData {
  ...
  return {
    ...
    image: ((input.image as string | null | undefined) ?? (input.image_url as string | null | undefined)) ?? null,
    ...
    tags: normalizeIngredients(input.tags),
